...
(function () {
  ...
  /* CORS-прокси в порядке приоритета — пробуем по очереди */
  var PROXY_LIST = [
  ...
  var REQUEST_TIMEOUT = 8000; // 8 сек таймаут

  function buildTargetUrl(path, params) {
    var qs = Object.keys(params || {}).map(function (k) {
      return k + '=' + encodeURIComponent(params[k]);
    }).join('&');
    return DEEZER_API + path + (qs ? '?' + qs : '');
  }
  ...
  /* Запрос через Lampa.Reguest с таймаутом */
  function requestViaLampa(targetUrl) {
    return new Promise(function (resolve, reject) {
      var done = false;
      var timer = setTimeout(function () {
        if (!done) { done = true; reject(new Error('lampa timeout')); }
      }, REQUEST_TIMEOUT);

      try {
        var req = new window.Lampa.Reguest();
        req.timeout(REQUEST_TIMEOUT);
        req.silent(targetUrl,
          function (data) {
            clearTimeout(timer);
            if (done) return;
            done = true;
          ...
          function (err) {
            clearTimeout(timer);
            if (!done) { done = true; reject(new Error(err || 'lampa error')); }
          }
        );
      } catch(e) {
        clearTimeout(timer);
  ...
  /* Запрос через fetch + CORS-прокси, перебираем все прокси */
  function requestViaFetch(targetUrl, attempt) {
    attempt = attempt || 0;
    if (attempt >= PROXY_LIST.length) {
      return Promise.reject(new Error('Все прокси недоступны'));
    }
    var proxyUrl = PROXY_LIST[attempt] + encodeURIComponent(targetUrl);
    return fetchWithTimeout(proxyUrl, REQUEST_TIMEOUT)
      .then(function (data) {
        if (data && data.error) throw new Error(data.error.message || 'Deezer error');
        if (!data || (!data.data && !data.id && !data.name && !data.title)) {
          throw new Error('empty response');
        }
        _proxyIndex = attempt; // запомнить рабочий прокси
        return data;
      })
      .catch(function () {
        return requestViaFetch(targetUrl, attempt + 1);
      });
  }

  /* Главный метод: Lampa.Reguest → fetch+прокси */
  function deezerGet(path, params) {
    var targetUrl = buildTargetUrl(path, params);
    if (window.Lampa && window.Lampa.Reguest) {
      return requestViaLampa(targetUrl).catch(function () {
        return requestViaFetch(targetUrl, 0);
      });
    }
    return requestViaFetch(targetUrl, 0);
  }
  ...
  var UI = {
    ...
    /* Блокируем перехват клавиш Lampa пока фокус на input/textarea */
    trapKeys: function () {
      var self = this;
      ...
      /* Перехватываем на capture-фазе — раньше любого обработчика Lampa */
      ['keydown', 'keyup', 'keypress'].forEach(function (ev) {
        document.addEventListener(ev, stop, true);
      });

      /* Отключаем контроллер Lampa пока открыт плагин */
      try {
    ...
    hide: function () {
      ...
      /* Снимаем перехват клавиш */
      if (this._keyTrap) {
        ['keydown', 'keyup', 'keypress'].forEach(function (ev) {
          document.removeEventListener(ev, this._keyTrap, true);
        }.bind(this));
        this._keyTrap = null;
      }
      /* Возвращаем управление Lampa */
      try {
        if (window.Lampa && Lampa.Controller) {
    ...
    renderHome: function () {
      ...
      inp.onclick = function () { inp.focus(); };
      ...
      var chartTimer = setTimeout(function () {
        if (grid.querySelector('.mt-spin')) {
          grid.innerHTML = '<div style="color:#cf6679;padding:12px;">Превышено время ожидания. Проверьте соединение.</div>';
        }
      }, 12000);
      ...
      deezerGet('/chart/0/tracks', { limit: 20 }).then(function (data) {
        ...
        clearTimeout(chartTimer);
        grid.innerHTML = '<div style="color:#cf6679;padding:12px;">Не удалось загрузить чарты: ' + (e.message || e) + '</div>';
      });
    },
    ...
    doSearch: function (q) {
      ...
      function renderTab(idx) {
        ...
        /* Страховочный таймаут: если запрос завис — показываем ошибку */
        var safetyTimer = setTimeout(function () {
          if (resultsArea.querySelector('.mt-spin')) {
            resultsArea.innerHTML = '<div style="color:#cf6679;padding:16px;">Превышено время ожидания. Проверьте соединение.</div>';
          }
        }, 12000);
        ...
        deezerGet(paths[idx], { q: q, limit: 30 }).then(function (data) {
          ...
          clearTimeout(safetyTimer);
          resultsArea.innerHTML = '<div style="color:#cf6679;padding:16px;">Ошибка загрузки: ' + (e.message || e) + '</div>';
        });
      }
    ...
    renderAlbum: function (id) {
      ...
      var albumTimer = setTimeout(function () {
        if (content.textContent.indexOf('Загрузка') !== -1) {
          content.innerHTML = '<div style="color:#cf6679;padding:16px;">Превышено время ожидания.</div>';
        }
      }, 12000);
      ...
      deezerGet('/album/' + id).then(function (album) {
        ...
        content.innerHTML = '<div style="color:#cf6679;padding:16px;">Ошибка загрузки альбома: ' + (e.message || e) + '</div>';
      });
    },
    ...
    renderArtist: function (id) {
      ...
      var artistTimer = setTimeout(function () {
        if (content.textContent.indexOf('Загрузка') !== -1) {
          content.innerHTML = '<div style="color:#cf6679;padding:16px;">Превышено время ожидания.</div>';
        }
      }, 12000);
      ...
      Promise.all([
        ...
        content.innerHTML = '<div style="color:#cf6679;padding:16px;">Ошибка загрузки артиста: ' + (e.message || e) + '</div>';
      });
    },
  ...
  var ICON_SVG = '<svg height="512" viewBox="0 0 512 512" width="512" xmlns="http://www.w3.org/2000/svg">'
    + '<path d="M480 224H352V96L160 128v256H32a32 32 0 0 0 0 64h128a32 32 0 0 0 32-32v-18.67L352 371V416h128a32 32 0 0 0 0-64H384V304h96a32 32 0 0 0 0-80z" fill="currentColor"/>'
    + '</svg>';

  /* ── Добавление пункта в боковое меню (все версии Lampa) ── */
  function addMenuItem() {
    var Lampa = window.Lampa;

    /* Способ 1: Lampa.Settings (наиболее универсальный, работает в большинстве сборок) */
    if (Lampa.Settings && Lampa.Settings.listener) {
      try {
        Lampa.Settings.listener.follow('open', function (e) {
          // не нужно, просто убеждаемся что объект живой
        });
      } catch (ignore) {}
    }

    /* Способ 2: через Component — стандарт для современных сборок */
    if (Lampa.Component) {
      try {
        Lampa.Component.add(PLUGIN_ID, {
          name: PLUGIN_NAME,
          icon: ICON_SVG,
          render: function () { UI.show(); return $('<div></div>'); },
          pause: function () {},
          resume: function () {},
          destroy: function () { UI.hide(); }
        });
      } catch (e) {
        console.warn('[MusicTorrent] Component.add:', e.message);
      }
    }

    /* Способ 3: прямая вставка li в DOM меню — работает всегда */
    function injectMenuDOM() {
      /* Ищем боковое меню: .menu__list или .menu ul */
      ...
      /* Не добавлять дважды */
      if (document.getElementById(PLUGIN_ID + '_menuitem')) return true;

      var li = document.createElement('li');
      li.id = PLUGIN_ID + '_menuitem';
      li.className = 'menu__item selector';
      li.style.cssText = 'cursor:pointer;';
      li.innerHTML = '<div class="menu__ico" style="display:flex;align-items:center;justify-content:center;">'
        + ICON_SVG
        + '</div>'
        + '<div class="menu__text">' + PLUGIN_NAME + '</div>';

      li.addEventListener('click', function () { UI.show(); });

      /* Lampa использует события hover:enter для навигации пультом */
      ...
      /* Пытаемся вставить перед последним пунктом меню */
      var items = menuList.querySelectorAll('.menu__item');
      if (items.length) {
        menuList.insertBefore(li, items[items.length - 1]);
      } else {
        menuList.appendChild(li);
      }

      /* Привязка Lampa-навигации если есть */
      if (Lampa.Controller && Lampa.Controller.enabled) {
        try { Lampa.Controller.toggle('menu'); } catch (ignore) {}
      }

      return true;
    }

    /* Пробуем вставить сразу */
    if (!injectMenuDOM()) {
      /* Если меню ещё не отрисовано — ждём через MutationObserver */
      var observer = new MutationObserver(function () {
        if (injectMenuDOM()) observer.disconnect();
      });
  ...
  function bindLampaEvents() {
    ...
    /* Некоторые сборки сигналят через 'start' напрямую */
  ...
  function registerPlugin() {
    ...
    /* Если app уже запущен (плагин подключён позже старта) */
    var appReady = false;
    try { appReady = !!document.querySelector('.menu__list'); } catch (ignore) {}
    if (appReady) addMenuItem();
  }
