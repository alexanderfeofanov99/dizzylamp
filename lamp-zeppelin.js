...
(function () {
  ...
  /* Строим итоговый URL с учётом прокси */
  function buildUrl(path, params) {
    var qs = Object.keys(params || {}).map(function (k) {
      return k + '=' + encodeURIComponent(params[k]);
    }).join('&');
    var target = DEEZER_API + path + (qs ? '?' + qs : '');
    return PROXY_LIST[_proxyIndex] + encodeURIComponent(target);
  }

  /* Запрос через Lampa.Reguest (нативный HTTP, без CORS) */
  function requestViaLampa(path, params) {
    return new Promise(function (resolve, reject) {
      var qs = Object.keys(params || {}).map(function (k) {
        return k + '=' + encodeURIComponent(params[k]);
      }).join('&');
      var url = DEEZER_API + path + (qs ? '?' + qs : '');
      ...
      req.silent(url, function (data) {
        ...
        reject(new Error(err || 'Lampa.Reguest failed'));
      });
    });
  }

  /* Запрос через fetch + CORS-прокси с авто-переключением */
  function requestViaFetch(path, params, attempt) {
    attempt = attempt || 0;
    ...
    return fetch(url, { method: 'GET', headers: { 'Accept': 'application/json' } })
      ...
      .catch(function () {
        /* Следующий прокси */
        _proxyIndex = (attempt + 1) % PROXY_LIST.length;
        return requestViaFetch(path, params, attempt + 1);
      });
  }

  /* Главный метод: сначала Lampa.Reguest (нет CORS), иначе fetch+прокси */
  function deezerGet(path, params) {
    if (window.Lampa && window.Lampa.Reguest) {
      return requestViaLampa(path, params).catch(function () {
        return requestViaFetch(path, params, 0);
      });
    }
    return requestViaFetch(path, params, 0);
  }
